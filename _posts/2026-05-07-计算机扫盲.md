...
*在文件资源管理器中，默认情况下是不显示扩展名的。需要在上方任务栏中的 查看→显示→文件扩展名（Windows系统） 勾上才能看见。隐藏文件同理，但是不建议勾选隐藏文件。*
...
ret2shellcode、ret2libc、ROP、栈迁移、tcache、fastbin dup、UAF、bin 结构、泄露、任意地址写、IO_FILE
