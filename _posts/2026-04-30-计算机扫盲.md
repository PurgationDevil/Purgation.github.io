...
*（大概知道有这样一个东西就行，又不是学操作系统的🥱。可以上下文反复多对比着看，也可以配合我的文章去问ai。）*
...
flowchart LR
    ...
    subgraph KernelMode [内核态 Kernel Mode]
        ...
        subgraph Kernel [内核 Kernel]
            direction LR
            K["线程调度 | 中断/异常处理 | 同步对象 | 时钟/定时器"]
        end
        
        subgraph HAL [硬件抽象层 HAL]
            direction LR
            H["屏蔽硬件差异 | 中断控制器 | DMA | 时钟管理"]
        end
    end
...
| :--------------- | :----------------------- | :---------------- | :------------------------- |
| 通用寄存器       | `edi, edx, ecx`          | `rdi, rdx, rcx`   | 存中间结果                 |
| 通用寄存器       | `eax`                    | `rax`             | 存返回值                   |
| 通用寄存器       | `ebx`                    | `rbx`             | 存地址                     |
...
```txt
[Nginx] 通过 FastCGI 协议 ↔ [PHP-FPM Master]
                              │
          ┌────────┬─────────┼─────────┬────────┐
          ▼        ▼         ▼         ▼        ▼
      [Worker] [Worker] [Worker] [Worker] [Worker]  (处理PHP请求)
```
